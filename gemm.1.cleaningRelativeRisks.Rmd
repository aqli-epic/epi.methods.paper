...
# create pm buckets 
pm_buckets_vec <- seq(0, 121, 1)

for(i in 1 : length(cause_ids_single_row)){
  # create a temp dataframe of with 2 columns: age_buckets_ll, pm_buckets
tmp_df <- tibble(age_buckets_ll = rep(age_buckets_ll_vec, times = length(pm_buckets_vec)), 
                 pm_buckets = rep(pm_buckets_vec, each = length(age_buckets_ll_vec)))
...
# create a dataframe in which we have a complete age range row set (from 25 to 95) for all pm buckets. Given that their are, 15 age buckets and 122 pm buckets, this will result in a final dataset of 15*122 = 1830 rows.


# start the expanding process, with the causes with cause id: 509, 426, 322 (ones which only have 1 row in the raw dataset)
